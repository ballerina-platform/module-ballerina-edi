...
// Regression test for the known first-occurrence limitation:
// When LoopTypeA is entirely absent from the input, the parser cannot route ENT*B
// to LoopTypeB on the first encounter — it has no stored qualifier to compare against.
// This test documents the gap so the behaviour is visible rather than masked.
@test:Config
