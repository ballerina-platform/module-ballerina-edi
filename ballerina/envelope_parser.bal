...
isolated function getEnvelopeOrError(EdiSchema schema) returns EdiEnvelopeSchema|Error {
    ...
    if env is () {
        return error SchemaCompatibilityError(string `Schema '${schema.name}' has no envelope defined. ` +
                "Envelope-aware APIs require a schema generated by edi-tools 2.2.0 or later " +
                "(X12: convertX12Schema; EDIFACT: convertEdifactSchema). " +
                "If the schema JSON was edited by hand, ensure the top-level 'envelope' field is present.");
    }
    return env;
}
